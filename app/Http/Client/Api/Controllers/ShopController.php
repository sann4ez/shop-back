...
//                'promotions',
    ...
    public function variation(Request $request, ProductVariation $variation)
    {
        ...
        $variation = ProductVariation::whereSlug($variation->slug)->byAllowed()->whereHas('product', fn($p) => $p->byDomain())
            ->with(['translations', 'media', 'product.translations', 'product.markers.translations',
                'properties.translations', 'properties.media', 'properties.attribute.translations',
//                'product.properties.translations', 'product.properties.attribute.translations',     // для таблиці характеристик (getSpecificationsArray)
//                'product.category.translations', 'product.category.promotions.translations', 'product.brand.translations',
                'promotions.translations',
            ])->firstOrFail();

        /** @var Product $product */
        $product = $variation->product->loadTrans([
            'category.translations', 'productparity.translations', 'category.promotions', 'productmodel.translations', 'brand.translations', 'tags.translations', 'media.model',

            'variations.translations', 'variations.properties.translations', 'variations.properties.attribute.translations',
            //'variations.properties.attribute.properties.translations',
            'variations.product.translations',
            'variations.properties.media',      // для побудови переключалки (buildSwitchingArray), атрибути з картинками

            'properties.translations', 'properties.attribute.translations',     // для таблиці характеристик (getSpecificationsArray)
            'category.promotions.translations', 'brand.promotions.translations', //  ціни зі знижками
        ])->checkAllowed();

        $additional = [
            'crumbs' => $variation->getBreadcrumbs(),      // TermSimpleResource::collection($product->category->ancestors->load('translations')), // TODO                                               // хлібні крихти категорія->підкатегорія->товар
        ];

        // перемикання варіацій
        $builder = new VariationBuilder($product->variations->all());
        $switching = $builder->buildSwitchingArray($variation);
        $additional['switching'] = $switching;

        // всі варіації групи
//        if (\Domain::getOptIs('variations.fields.variations')) {
//            ProductVariationSwitchingResource::setCurrent($variation);
//            $additional['variations'] = ProductVariationSwitchingResource::collection($product->variations->load('media', 'translations'));   // варіації товару
//        }

        // повязані, супутні
        if (true) {
            $additional['relateds'] = ProductVariationListResource::collection($variation->getRelateds());
        }
//
//        // парні варіації (до поточного)
//        if (\Domain::getOptIs('products.fields.productparity')) {
//            $additional['parities'] = ProductVariationListResource::collection($variation->getParities());
//        }

        // рекомендовані, подібні
        if (true) {
            $additional['recommends'] = ProductVariationListResource::collection($variation->getRecommends(true));
        }
    ...
    /**
     *  @api {get} /api/shop/variations/revieweds 09. Переглянуті варіації
     *  @apiVersion 1.0.0
     *  @apiName ShopVariationsRevieweds
     *  @apiGroup Shop
     *
     *  @apiHeader {String} [sRevieweds] IDs: `234,56,678,89,567`
     *
     *  @apiDescription Отримати список переглянутих варіацій, ід яких передається в запиті з клієнта.
     *
     *  @apiParam {String} [sRevieweds] Рядок ід-ів варіацій через кому: `234,56,678,89,567`
     *
     *  @apiSuccessExample {json} Response-Success: HTTP/1.1 200 OK
     *  {
     *      "data": [
     *          {
     *              "id": "366cf4d1-7d78-4b6b-9539-4ce34eeda902",
     *              "slug": "nich-druha",
     *              "name": "Цукерки",
     *              "sku": "5000007",
     *              "status": "published",
     ...
     *                          {
     *                              "property": {
     *                                  "id": "223626e8-9f98-4812-a273-aa2a8eb037f1",
     *                                  "slug": "red",
     *                                  "color": null,
     *                                  "image": "https://dropshop.demka.online/storage/16c11db6-513f-47a9-adf8-6858d0a15b32/red.png",
     *                                  "value": "Червоний"
     *                              },
     *                              "variation": {
     *                                  "id": "366cf4d1-7d78-4b6b-9539-4ce34eeda902",
     *                                  "url": "#",
     *                                  "name": "Цукерки Ніч Червона",
     *                                  "slug": "nich-druha",
     *                                  "stock_qty": 10
     *                              },
     *                              "is_current": true
     ...
     *              {
     *                  "attribute": {
     *                      "id": "2ddeb07d-5862-493c-bacf-75190e7147e7",
     *                      "name": "Стать",
     *                      "slug": "gender",
     *                      "format": "text",
     *                      "has_image": false
     ...
     *                      {
     *                          "property": {
     *                              "id": "17cc74d4-0c3a-4a00-92a1-9528e16eba27",
     *                              "slug": "female",
     *                              "color": null,
     *                              "image": "",
     *                              "value": "Female"
     ...
     *              {
     *                  "attribute": {
     *                      "id": "e93520fc-4ebe-4af7-8ac8-962a6cc59b72",
     *                      "name": "Батарея",
     *                      "slug": "battery",
     *                      "format": "text",
     *                      "has_image": false
     ...
     *                      {
     *                          "property": {
     *                              "id": "1f1c0627-857f-4c48-b8b6-b14bc038a40d",
     *                              "slug": "4000",
     *                              "color": null,
     *                              "image": "",
     *                              "value": "4000 mA*H"
     ...
     *              "images": [
     *                  {
     *                      "id": "dbd957e4-86da-4610-9aed-b3a642cb2768",
     *                      "name": "Неонова дівчина (1920x1080)",
     *                      "url": "https://dropshop.demka.online/storage/dbd957e4-86da-4610-9aed-b3a642cb2768/neonova-divcina-1920x1080.jpg",
     *                      "conversions": {"big": { "url": "..." },"thumb": { "url": "..." },"preview": { "url": "..." }}
     *                  }
     *              ],
     *              "product": {
     *                  "id": "6d840be8-4d70-4b2a-8046-f591732bc5da",
     *                  "rating": 2.33,
     *                  "comments_count": 3,
     *                  "income_at": "2024-05-28T14:01:00.000000Z",
     *                  "productparity": null,
     *                  "category": {
     *                      "id": "fa7561d7-1e5c-4e45-8183-b65c7740bc7e",
     *                      "name": "Столи",
     *                      "slug": "stoly",
     *                      "variations_count": 35
     *                  },
     *                  "brand": {
     *                      "id": "423a28dc-0639-4e9f-ac45-30d7fab135f7",
     *                      "name": "Тигрес",
     *                      "slug": "tyhres",
     *                      "variations_count": 0
     *                  }
     ...
     *      ],
     *      "links": {
     *          "first": "https://dropshop.demka.online/api/shop/variations/revieweds?page=1",
     *          "last": "https://dropshop.demka.online/api/shop/variations/revieweds?page=1",
     *          "prev": null,
     *          "next": null
     *      },
     *      "meta": {
     ...
     *          "links": [
     *              {
     *                  "url": "https://dropshop.demka.online/api/shop/variations/revieweds?page=1",
     *                  "label": "1",
     *                  "active": true
     *              },
     *              {
     ...
     *              }
     *          ],
     *          "path": "https://dropshop.demka.online/api/shop/variations/revieweds",
     *          "per_page": 16,
     *          "to": 1,
     *          "total": 1
     *      }
     *  }
     */
    public function revieweds(Request $request)
    {
        $variations = ProductVariation::with([
            'translations', 'media.model',
            'product', 'product.translations', 'product.media.model', 'product.category', 'product.category.promotions', 'product.productparity.translations',
            'properties.translations', 'properties.media', 'properties.attribute.translations',
            'product.category.translations', 'product.brand.translations', 'product.markers.translations',
            'promotions.translations', 'product.category.promotions.translations', 'product.brand.promotions.translations', //  ціни зі знижками
        ])->byRevieweds()
            ->filterable($request->all(), ['status' => ProductVariation::STATUS_PUBLISHED, 'sort_reality' => 1])
            ->paginate();
