...
namespace App\Support\Shippings\Novaposhta;

use App\Services\Novaposhta\NovaposhtaAreas;
...
class Novaposhta
{
     ...
     * @return $this
     */
    public function __construct($key = null, $language = 'uk', $throwErrors = false, $connectionType = 'curl')
    {
        $key = $key ?: \Variable::getArray('shipping.novaposhta.api_key', null);
    ...
    private function request($model, $method, $params = null)
    {
        ...
        if ('curl' == $this->getConnectionType()) {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: ' . ('xml' == $this->format ? 'text/xml' : 'application/json')]);
     ...
     * @throws \Exception
     */
    public function getStatusDocuments(string $track, string $phone = '')
    {
        $params = ['Documents' => [
                [
    ...
    public function getSettlements(array $attrs)
    {
        return $this->request('AddressGeneral', 'getSettlements', $attrs);
    }

    public function searchSettlements($cityName = '', $limit = 50)
    {
     ...
     * @throws \Exception
     */
    public function getWarehouses(array $attrs)
    {
        return $this->request('Address', 'getWarehouses', $attrs);
    }

    public function getWarehousesByTypes(array $types, array $attrs = [])
    {
        $res = [];

        $cacheName = md5(serialize($attrs));
        $apiRes = Cache::remember($cacheName, 3600, function () use ($attrs) {
            return $this->request('Address', 'getWarehouses', $attrs);
        });
        //$apiRes = $this->request('Address', 'getWarehouses', $attrs);

        $typeIds = [];
        foreach ($types as $type) {
            switch ($type) {
                case 'office':
                    $typeIds[] = '841339c7-591a-42e2-8233-7a0a00f0ed6f';
                    $typeIds[] = '9a68df70-0267-42a8-bb5c-37f427e36ee4';
                break;
                case 'cargo':
                    $typeIds[] = '9a68df70-0267-42a8-bb5c-37f427e36ee4';
                break;
                case 'locker':
                    $typeIds[] = 'f9316480-5f2d-425d-bc2c-ac7cd29decf0';
                break;
                case 'locker_pb':
                    $typeIds[] = '95dc212d-479c-4ffb-a8ab-8c1b9073d0bc';
                break;
                case 'parcel':
                    $typeIds[] = '6f8c7162-4b72-4b0a-88e5-906948c6a92f';
                break;
            }
        }
        ...
        if (isset($apiRes['success']) && $apiRes['success'] === true) {
            if ($typeIds) {
                foreach ($apiRes['data'] as $item) {
                    if (in_array($item['TypeOfWarehouse'], $typeIds)) {
                        $res[] = $item;
                    }
                }
            } else {
    ...
    /**
     *
     * 'office' => '841339c7-591a-42e2-8233-7a0a00f0ed6f', // Поштове відділення (до 30кг)
     * 'cargo' => '9a68df70-0267-42a8-bb5c-37f427e36ee4', // Вантажне відділення
     * 'locker' => 'f9316480-5f2d-425d-bc2c-ac7cd29decf0', // Поштомат
     * //'locker_pb' => '95dc212d-479c-4ffb-a8ab-8c1b9073d0bc',  // Поштомат ПриватБанку
     * 'parcel' => '6f8c7162-4b72-4b0a-88e5-906948c6a92f', // Відділення поштове до 10 кг
     *
     * @param array $attrs
     * @return array|mixed
     * @throws \Exception
     */
    public function getWarehouseTypes(array $attrs)
    {
    ...
    public function getWarehouse($cityRef, $description = '')
    {
        ...
        // Error
        $error = empty($data) ? ['Warehouse was not found'] : [];
    ...
    public function getArea($findByString = '', $ref = '')
    {
        ...
        empty($this->areas) and $this->areas = NovaposhtaAreas::getAreas();

        $data = $this->findArea($this->areas, $findByString, $ref);

        // Error
        $error = empty($data) ? 'Area was not found' : null;
    ...
    public function getCity($cityName, $areaName = '')
    {
        ...
        // Error
        $error = empty($data) ? 'City was not found' : null;
     ...
     * @return mixed
     */
    public function getDocumentPrice(array $attrs)
    {
        return $this->request('InternetDocument', 'getDocumentPrice', $attrs);
    }
    /**
...
//        var_dump($sender['CityRef']);die;
    ...
    public static function formatSchedule(array $days = []): string
    {
        $replacement = [
            'Monday' => 'Понедельник',
            'Tuesday' => 'Вторник',
