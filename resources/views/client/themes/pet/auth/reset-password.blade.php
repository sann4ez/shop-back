            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-6 col-lg-7 col-xl-8 pt-3">
                        <span class="top-sub-title">RESET PASSWORD</span>
                        <h2 class="font-weight-bold text-4 mb-1">Password reset Now!</h2>
                        <p class="lead mb-4">&nbsp;</p>
                        @if (session('status'))
                        ...
                        <form action="{{ route('password.update') }}" method="post">
                            ...
                            <div class="form-row">
                                <div class="form-group col-lg-6">
                                    ...
                                    <input type="email" value="{{ old('email', $request->email) }}" class="form-control bg-light-5 border-0 rounded" name="email">
                                    @error('email') <label class="error">{{ $message }}</label> @enderror
                                </div>
                                <div class="form-group col-lg-6">
                                    <label class="text-color-dark" for="shopLoginRegisterEmail">NEW PASSWORD:</label>
                                    <input type="password" value="" class="form-control bg-light-5 border-0 rounded" name="password">
                                    @error('email') <label class="error">{{ $message }}</label> @enderror
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="col text-right">
                                    <button type="submit" class="btn btn-primary btn-rounded btn-v-3 btn-h-3 font-weight-bold">RESET NOW</button>
                                </div>
                            </div>
                        </form>
                    </div>
